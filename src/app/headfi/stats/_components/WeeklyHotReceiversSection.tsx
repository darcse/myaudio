...
import { Crown, Flame, Headphones } from 'lucide-react';
import { ALBUM_COVER_BLUR_OVERLAY } from '@/app/albums/_components/AlbumCoverBlurBackdrop';
import type { GearListenRankItem, WeekRange } from '@/app/albums/stats/albumListenStats';
import { formatWeekRangeLabel } from '@/app/albums/stats/albumListenStats';

type WeeklyHotReceiversSectionProps = {
  items: GearListenRankItem[];
  weekRange: WeekRange;
  onGearClick: (headfiId: number) => void;
};

function HotReceiverCover({
  imageUrl,
  label,
}: {
...
function WeeklyHotReceiverCard({
  item,
  rank,
  onClick,
}: {
      ...
      >
        {item.imageUrl ? (
          ...
          <div
            ...
            aria-hidden
          >
            <Headphones className="size-12 opacity-40" strokeWidth={1.5} />
          </div>
        )}
        <div
        ...
        <div className="absolute inset-x-0 bottom-0 z-[2] p-3 sm:p-4">
          <p className="line-clamp-2 text-base font-semibold leading-snug drop-shadow-sm sm:text-lg">{name}</p>
          <p className="mt-0.5 truncate text-sm text-white/80 drop-shadow-sm">{item.category}</p>
  ...
  return (
    ...
    >
      ...
      <HotReceiverCover imageUrl={item.imageUrl} label={name} />
...
export function WeeklyHotReceiversSection({
  items,
  weekRange,
  onGearClick,
}: WeeklyHotReceiversSectionProps) {
      ...
      >
        {items.length === 0 ? (
          <div className="px-6 py-12 text-center">
            <p className="text-sm font-medium opacity-80">최근 7일 리시버 사용 기록이 없습니다.</p>
          ...
          <div className="grid gap-4 p-4 sm:grid-cols-2 sm:items-stretch sm:p-5">
            {first ? (
              <WeeklyHotReceiverCard
                item={first}
                rank={1}
                onClick={() => onGearClick(first.headfiId)}
              />
            ) : null}
            {rest.length > 0 ? (
