...
export async function recommendAlbumHeadphones(
  ...
  const prompt = `너는 헤드파이 전문 리뷰어야. 실제 기기 특성과 리뷰를 참고해서 이 앨범에 가장 잘 어울리는 리시버(헤드폰·이어폰)를 추천해줘.
...
가장 잘 어울리는 리시버 최대 2개를 선택하고, 음향적 근거를 포함한 추천 이유를 작성해줘.

후보 리시버가 2개 이상이면 headphone_ids 배열에 반드시 정확히 2개의 id를 넣어.
headphone_ids에는 위 목록에 있는 id 숫자만 사용하고, reason에 언급하는 리시버와 headphone_ids가 반드시 일치해야 해.
reason에는 브랜드·모델명만 쓰고 id나 괄호 안 숫자는 절대 넣지 마.

평가 기준:
- 장르와 리시버 음색의 매칭도
- 리시버의 강점(저역/중역/고역)이 이 앨범의 핵심 사운드와 부합하는지
- 실제 측정/리뷰 데이터 기반 신뢰성
